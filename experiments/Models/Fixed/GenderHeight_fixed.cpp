...
int main() {
    ...
    std::vector<Fixed> gender_means(NUM_ITERS);
    ...
    for (int i = 0; i < NUM_ITERS; ++i) {
        try {
            ...
            std::vector<Fixed> gender_samples(NUM_SAMPLES);

            ppl::Data<Fixed> height {64.5, 65};
            ppl::Param<Fixed> gender {gender_samples.data()};

            auto model = (
                gender |= ppl::bernoulli(Fixed(0.5)),
                height |= ppl::normal(gender*Fixed(64.)+(Fixed(1.)-gender)*Fixed(68.), Fixed(3.))
            );
