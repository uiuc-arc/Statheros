...
int main() {
    ...
    for (int i = 0; i < NUM_ITERS; ++i) {
        try {
            ...
            std::vector<Fixed> coin1_samples(NUM_SAMPLES),coin2_samples(NUM_SAMPLES);
    
            ppl::Data<Fixed> obs {1.0};
            ppl::Param<Fixed> coin1 {coin1_samples.data()};
            ppl::Param<Fixed> coin2 {coin2_samples.data()};

            auto model = (
                coin1 |= ppl::bernoulli(Fixed(0.5)),
                coin2 |= ppl::bernoulli(Fixed(0.5)),
                obs |= ppl::bernoulli((coin1+coin2)>Fixed(0.))
            );
