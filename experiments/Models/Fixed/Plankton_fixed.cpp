...
int main() {
    ...
    std::vector<Fixed> n_param_means(NUM_ITERS);
    ...
    for (int i = 0; i < NUM_ITERS; ++i) {
        try {
            ...
            std::vector<Fixed> n_param_samples(NUM_SAMPLES);

            ppl::Data<Fixed> plankton {4.667,8.814, 9.088}; //8.814, 9.088, 7.339
            ppl::Param<Fixed> n_param {n_param_samples.data()};

            auto model = (
                n_param |= ppl::uniform(Fixed(5.), Fixed(50.)),
                plankton |= ppl::normal(Fixed(0.5)*n_param, Sqrt(Fixed(0.25),n_param))
            );
