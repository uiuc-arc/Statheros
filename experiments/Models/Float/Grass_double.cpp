...
int main() {
    ...
    for (int i = 0; i < NUM_ITERS; ++i) {
        try {
            ...
            std::vector<disc_t> cloudy_samples(NUM_SAMPLES),
                                sprinkler_samples(NUM_SAMPLES),
                                rain_samples(NUM_SAMPLES);

            ppl::Param<disc_t>  cloudy {cloudy_samples.data()}, 
                                sprinkler {sprinkler_samples.data()},
                                rain {rain_samples.data()};
            ppl::Data<disc_t> wet_grass;
            ...
            //std::cout << "time: " << (t_after_mh - t_before_mh).count() << std::endl;
