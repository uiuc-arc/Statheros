...
int main() {
    ...
    std::vector<cont_t>  cloudy_means(NUM_ITERS),
    ...
    for (int i = 0; i < NUM_ITERS; ++i) {
        try {
            ...
            std::vector<int> cloudy_samples(NUM_SAMPLES),
                            sprinkler_samples(NUM_SAMPLES),
                            rain_samples(NUM_SAMPLES);

            ppl::Param<int> cloudy {cloudy_samples.data()}, 
                            sprinkler {sprinkler_samples.data()},
                            rain {rain_samples.data()};
            ppl::Data<int> wet_grass;

            auto model = (
                cloudy |= ppl::bernoulli(cont_t(0.5)),

		sprinkler |= ppl::bernoulli(ppl::mix2(cloudy , cont_t(0.1) , (1. - cloudy) , cont_t(0.5))),
                //sprinkler |= ppl::bernoulli(cloudy * cont_t(0.1) + (1 - cloudy) * cont_t(0.5)),

		rain |= ppl::bernoulli(ppl::mix2(cloudy , cont_t(0.8) , (1. - cloudy) , cont_t(0.2))),
                //rain |= ppl::bernoulli(cloudy * cont_t(0.8) + (1 - cloudy) * cont_t(0.2)),
                /*wet_grass |= ppl::bernoulli((sprinkler * rain) * cont_t(0.99)
                                        + (sprinkler * (1 - rain)) * cont_t(0.9)
                                        + ((1 - sprinkler) * rain) * cont_t(0.9)
                                        + ((1 - sprinkler) * (1 - rain)) * cont_t(0.01))*/

		wet_grass |= ppl::bernoulli(ppl::mix4((sprinkler * rain), cont_t(0.99)
                                        , (sprinkler * (1 - rain)) , cont_t(0.9)
                                        , ((1 - sprinkler) * rain) , cont_t(0.9)
                                        , ((1 - sprinkler) * (1 - rain)) , cont_t(0.01)))
