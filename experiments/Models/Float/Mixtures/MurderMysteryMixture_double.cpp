...
int main() {
    ...
    for (int i = 0; i < NUM_ITERS; ++i) {
        try {
            ...
            std::vector<int> aliceDunnit_samples(NUM_SAMPLES);

            ppl::Data<int> withGun;
            ppl::Param<int> aliceDunnit {aliceDunnit_samples.data()};

            auto model = (
                aliceDunnit |= ppl::bernoulli(cont_t(0.3)),

		withGun |= ppl::bernoulli(ppl::mix2(aliceDunnit , cont_t(0.03) , (1. - aliceDunnit) , cont_t(0.8)))
                //withGun |= ppl::bernoulli(aliceDunnit * cont_t(0.03) + (1 - aliceDunnit) * cont_t(0.8))
            );
