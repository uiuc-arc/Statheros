...
int main() {
    std::cout << "Coin1Probability,acceptance_ratio,time_elapsed_us,start_time,end_time,mh_start_time,mh_end_time" << std::endl;
    ...
    for (int i = 0; i < NUM_ITERS; ++i) {
        try {
            ...
            std::vector<disc_t> coin1_samples(NUM_SAMPLES), coin2_samples(NUM_SAMPLES);
  
            ppl::Data<disc_t> obs {1};
            ppl::Param<disc_t> coin1 {coin1_samples.data()};
            ppl::Param<disc_t> coin2 {coin2_samples.data()};

            auto model = (
                coin1 |= ppl::bernoulli(cont_t(0.5)),
                coin2 |= ppl::bernoulli(cont_t(0.5)),
                obs |= ppl::bernoulli((coin1+coin2)>cont_t(0.))
            );
            ...
            auto param_mean = ppl::math::mean(coin1_samples);
