...
int main() {
    ...
    std::vector<cont_t> param_means(NUM_ITERS);
    ...
    for (int i = 0; i < NUM_ITERS; ++i) {
        try {
            ...
            std::vector<cont_t> param_samples(NUM_SAMPLES);

            ppl::Data<disc_t> is_success1 {(1)}; // location 1
            ppl::Data<disc_t> is_success2 {(1)}; // location 2
            ppl::Data<disc_t> is_success3 {(1)}; // location 3
            ppl::Data<disc_t> is_success4 {(1)}; // location 4
            ppl::Data<disc_t> is_success5 {(1)}; // location 5
            ppl::Data<disc_t> is_success6 {(1)}; // location 6
            ppl::Data<disc_t> is_success7 {(1)}; // location 7
            ppl::Data<disc_t> is_success8 {(1)}; // location 8
            ppl::Data<disc_t> is_success9 {(1)}; // location 9
            ppl::Data<disc_t> is_success10 {(1)}; // location 10
            ppl::Param<cont_t> param {param_samples.data()};

            auto model = (
                param |= ppl::uniform(cont_t(0.), cont_t(1.)),
