...
struct continuous_dist_base {};

//Added to leverage type_traits std::is_base_of()
struct discrete_dist_base {};
 ...
 */
template <class T, class dist_value_t_>
struct DistExpr : BaseCRTP<T>
{ 
    using BaseCRTP<T>::self;
    using dist_value_t = dist_value_t_;
    ...
    dist_value_t log_pdf(const VarType& v) const 
        ...
        // by static analysis, we guarantee that self().log_pdf(v.get_value(i), i)
        // will not overflow
	...
	return DeterministicInterval(0, 1); //dummy
    }

    void printName(){
	std::cout << "some distribution" << std::endl;
    }


   void log_pdf_interval(DeterministicInterval& likelihoodInterval, DeterministicInterval& biggestInterval){}

    virtual std::set<boost::uuids::uuid> getDeps(){} //virtual doesn't work
...
inline constexpr bool dist_expr_is_base_of_v =
    std::is_base_of_v<DistExpr<T, typename T::dist_value_t>, T>;
